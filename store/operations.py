...
def set_sold_amount(client: AlgodClient, creator: Account, app_id: int, account: Account, amount: int):
    call_txn = transaction.ApplicationCallTxn(
        sender=creator.get_address(),
        sp=client.suggested_params(),
        index=app_id,
...
def set_bought_amount(client: AlgodClient, creator: Account, app_id: int, account: Account, amount: int):
    call_txn = transaction.ApplicationCallTxn(
        sender=creator.get_address(),
        sp=client.suggested_params(),
        index=app_id,
...
def buy(client: AlgodClient, app_id: int, seller: Account, buyer: Account, amount: int):
    call_txn = transaction.ApplicationCallTxn(
    ...
    signed_txn = call_txn.sign(buyer.get_private_key())
    tx_id = client.send_transaction(signed_txn)
    wait_for_confirmation(client, tx_id)
    
def sell(client: AlgodClient, app_id: int, seller: Account, buyer: Account, amount: int):
    call_txn = transaction.ApplicationCallTxn(
    ...
    signed_txn = call_txn.sign(seller.get_private_key())
    tx_id = client.send_transaction(signed_txn)
    wait_for_confirmation(client, tx_id)
