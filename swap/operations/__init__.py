...
def get_contracts(client: AlgodClient) -> Tuple[bytes, bytes]:
    approval_teal = importlib.resources.read_text(swap.contracts, 'validator_approval.teal')
    clear_teal = importlib.resources.read_text(swap.contracts, 'validator_clear_state.teal')
...
def get_pool_logicsig(client: AlgodClient, validator_app_id, asset1_id, asset2_id):
    teal = importlib.resources.read_text(swap.contracts, 'pool_logicsig.teal.tmpl')
